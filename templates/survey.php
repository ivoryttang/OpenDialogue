...
if ($_REQUEST['similar'] == null) {
    echo "<h1 style=\"text-align:center;\">Showing Survey Results of Opinions Different From Yours</h1>";
}
else{
    echo "<h1 style=\"text-align:center;\">Showing Survey Results of Opinions Similar To Yours</h1>";
}
...
$polarity_race = -$_POST['racism'];
$polarity_sex = -$_POST['sexism'];
$polarity_orient = -$_POST['sexual_orientation'];
if ($polarity_race <= 0 && $polarity_sex <= 0 && $polarity_orient <= 0) {
    $sql = "SELECT * FROM od_tweet WHERE topic='Race' AND polarity<=0 OR topic='Gender' AND polarity<=0 OR topic='Sexual Orientation' AND polarity<=0 ORDER BY likes DESC LIMIT 50;";
}
else if ($polarity_race <= 0 && $polarity_sex <= 0 && $polarity_orient >= 0){
    $sql = "SELECT * FROM od_tweet WHERE topic='Race' AND polarity<=0 OR topic='Gender' AND polarity<=0 OR topic='Sexual Orientation' AND polarity>=0 ORDER BY likes DESC LIMIT 50;";
}
else if ($polarity_race <= 0 && $polarity_sex >= 0 && $polarity_orient <= 0){
    $sql = "SELECT * FROM od_tweet WHERE topic='Race' AND polarity<=0 OR topic='Gender' AND polarity>=0 OR topic='Sexual Orientation' AND polarity<=0 ORDER BY likes DESC LIMIT 50;";
