...
$topic = $_POST['topic'];
echo "<h1 style=\"text-align:center;\">$topic Dialogue</h1>";
...
$sql = "SELECT id, username, created_at, text, likes, reply FROM od_tweet WHERE topic='$topic' ORDER BY id DESC";
        ...
        if (25 < strlen($row["created_at"]) ){
            ...
            if ($reply != "n"){
                ...
                echo "<form style='display:inline-block;' method='post'>
                    ...
                    </form>  <form style='display:inline-block;' method='post'>
                        <input type=\"hidden\" name=\"topic\" value=\"$topic\">
                        <input type=\"hidden\" name=\"id\" value='$id' />
                        <input type=\"hidden\" name=\"likes\" value='$likes' />
                        <input type=\"submit\" name=\"$id\" value='Reply'/></form>";
                if (isset($_POST[$id])){
                ...
                foreach ($replies_array as &$value){
                    if ($value != ""){
                        ...
                        while($row = pg_fetch_assoc($result_update)) {
                            ...
                            echo "<form style='position:relative; left:60px; width: 95%; display:inline-block;' method='post' action='http://127.0.0.1:3338/blm.php'>
                                    <input type=\"hidden\" name=\"id\" value='$id' />
                                    <input type=\"hidden\" name=\"likes\" value='$likes' />
                                    <input type=\"submit\" name=\"Like\" value=\"Like\" />
                                </form>  <form style='display:inline-block; position:relative; left:-1090px' method='post'><input type=\"submit\" name=\"$id\" value=\"Reply\"/></form>";
                            if (isset($_POST[$id])){
                                echo "<form action='http://127.0.0.1:3338/blm.php' method=\"post\">
                                  <input style=\"height:100px; width:1000px; \" type=\"text\" name=\"text\"><br>
                                  <input type=\"hidden\" name=\"topic\" value=\"Black Lives Matter\">
                                  <input type=\"hidden\" name=\"id\" value='$id'/>
                                  <input type=\"hidden\" name=\"reply\" value='$reply'/>
                                  <input type=\"hidden\" name=\"username2\" value=\"Anon\">
                                  <input type=\"submit\" name=\"Reply\">
            ...
            if ($reply != "n"){
                echo "<h3 style=\"background-color:powderblue;\"> Username: ".$row["username"]."</h3><h4 style=\"border: 1px solid grey; padding: 14px 16px; position:relative; left:30px; background-color:#faf6d2; border-radius: 15px;width: 90%;\">Date: " . $row["created_at"]."<br>".$row["text"]."<br>". $row["likes"]." &hearts;</h4>\n";
                ...
                foreach ($replies_array as &$value){
                    if ($value != "" && $value != "n"){
                        ...
                        while($row = pg_fetch_assoc($result_update)) {
                            ...
                            #likes
                            echo "<form method='post' style='display:inline-block; position:relative; left:90px;'>
                                    <input type=\"hidden\" name=\"topic\" value='$topic' />
                                    <input type=\"hidden\" name=\"id\" value='$id' />
                                    <input type=\"hidden\" name=\"likes\" value='$likes' />
                                    <input type=\"submit\" name=\"Like\" value=\"Like\" />
