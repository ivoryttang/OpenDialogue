...
echo "<h1 style=\"text-align:center;\">Black Lives Matter Dialogue</h1>";
...
$sql = "SELECT id, username, created_at, text, likes, reply FROM od_tweet WHERE topic='Black Lives Matter' ORDER BY id DESC";
...
if (0 < $row) {
    ...
    while($row = pg_fetch_assoc($result)) {
        $reply = $row['reply'];
        if (25 < strlen($row["created_at"]) ){ #from twitter (date is super long)
            #not a reply
            if ($reply != "n"){
                ...
                foreach ($replies_array as &$value){
                    if ($value != ""){
                        ...
                        while($row = pg_fetch_assoc($result_update)) {
                            ...
                            #likes and replies
                            echo "<form style='position:relative; left:90px; width: 95%; display:inline-block;' method='post' action='http://127.0.0.1:3338/blm.php'>
                                    <input type=\"hidden\" name=\"id\" value='$id' />
                                    <input type=\"hidden\" name=\"likes\" value='$likes' />
                                    <input type=\"submit\" name=\"Like\" value=\"Like\" />
                                </form>";
            ...
            if ($reply != "n"){
                ...
                #likes
                echo "<form method='post' style='display:inline-block;' action='http://127.0.0.1:3338/blm.php'>
                        <input type=\"hidden\" name=\"id\" value='$id' />
                        <input type=\"hidden\" name=\"likes\" value='$likes' />
                        <input type=\"submit\" name=\"Like\" value=\"Like\" />
                    </form>  <form style='display:inline-block;' method='post'><input type=\"submit\" name=\"$id\" value='Reply'/></form>";
